...
## Q1: Explain the CLIP InfoNCE (contrastive) loss function.

For a batch of $N$ (image, text) pairs, CLIP computes an $N \times N$ cosine similarity matrix $S$ where $S_{ij} = \mathbf{i}_i^\top \mathbf{t}_j / \tau$ and $\tau$ is a learnable temperature parameter. The **InfoNCE loss** applies cross-entropy over rows and columns: $\mathcal{L} = -\frac{1}{2N}\sum_{k=1}^N \left[\log \frac{e^{S_{kk}}}{\sum_j e^{S_{kj}}} + \log \frac{e^{S_{kk}}}{\sum_i e^{S_{ik}}}\right]$. The first term requires each image to identify its paired text from all $N$ texts; the second requires each text to identify its paired image from all $N$ images. InfoNCE is a lower bound on mutual information $I(\mathbf{i}; \mathbf{t})$; maximizing it pushes paired embeddings together and unpaired ones apart. Large batch sizes are critical — each sample is contrasted against $N-1$ negatives, and larger $N$ provides harder negatives and tighter MI estimates. CLIP trained with $N = 32{,}768$.

## Q2: How does ViT patch images? What is the patch embedding?

**ViT** (Vision Transformer) divides an input image of shape $(H, W, C)$ into a grid of non-overlapping $P \times P$ patches, yielding $n = HW/P^2$ patches (e.g., $224 \times 224$ image with $P=16$ → 196 patches). Each patch is flattened to a vector of dimension $P^2 C$ and linearly projected to the model dimension $d$ via a learned weight matrix $E \in \mathbb{R}^{(P^2 C) \times d}$: $\mathbf{z}_i = \text{flatten}(\text{patch}_i) \cdot E$. A learnable **CLS token** is prepended to the sequence, and **learnable 1D positional embeddings** are added to all $n+1$ tokens before the Transformer encoder. The patch projection is the ViT's analogue of a CNN's first convolutional layer — it learns to extract local features within each patch — but unlike convolutions, there is no weight sharing across spatial positions.

## Q3: What is cross-modal attention and how does it fuse image and text features?

**Cross-modal attention** is a Transformer attention layer where queries $Q$ come from one modality and keys $K$, values $V$ come from another: $\text{Attn}(Q, K, V) = \text{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right)V$. In a vision-language model, text token embeddings provide $Q$ while image patch embeddings provide $K$ and $V$; each text token computes attention weights over all image patches and aggregates their features as a weighted sum. This allows the model to ground language tokens in specific visual regions — "red" attends to high-saturation patches, "dog" attends to the animal region. Cross-modal attention is the core of architectures like Flamingo and LLaVA, enabling deep fusion where both modalities influence each other's representations, in contrast to **late fusion** which merges modality embeddings only at the final output stage.

## Q4: Explain zero-shot image classification using CLIP.

Zero-shot classification with CLIP requires no labeled training images for the target classes. For each class $c$ in the label set, construct a text prompt $t_c$ such as "a photo of a {class\_name}" and compute its text embedding $\mathbf{t}_c = f_\text{text}(t_c)$ using CLIP's text encoder. Encode the query image: $\mathbf{i} = f_\text{image}(x)$. Compute cosine similarities $s_c = \mathbf{i}^\top \mathbf{t}_c / (\|\mathbf{i}\|\|\mathbf{t}_c\|)$ for all classes. Apply softmax with temperature $\tau$: $p_c = e^{s_c/\tau} / \sum_{c'} e^{s_{c'}/\tau}$. Predict the class with highest $p_c$. This works because CLIP's joint embedding space aligns images and their natural-language descriptions — the model has never seen the target classes explicitly but has learned their visual-semantic associations from 400M image-text pairs.

## Q5: What is the role of the projection layer in CLIP?

Both CLIP's image encoder (ViT or ResNet) and text encoder (Transformer) independently produce high-dimensional representations in their own feature spaces. The **projection layer** is a learned linear (or MLP) head appended to each encoder that maps these representations into a shared **joint embedding space** of dimension $d_e$ (e.g., 512 or 1024). After projection, both modalities live in the same $\mathbb{R}^{d_e}$ space and embeddings are L2-normalized to unit norm, making cosine similarity equivalent to a dot product. Without the projection, the image and text feature spaces would have no reason to align — they are initialized differently and trained on different objectives. The projection layers are trained end-to-end by the InfoNCE contrastive loss, which explicitly drives paired image-text embeddings toward alignment.

## Q6: How does Flamingo handle interleaved image-text sequences?

**Flamingo** (Alayrac et al., 2022) processes sequences of the form `[image] ... [text] ... [image] ... [text]` by inserting **Perceiver Resampler** modules that compress each image's ViT features (variable number of patches) into a fixed number of visual tokens (e.g., 64). These visual tokens are injected into a frozen large language model via **gated cross-attention layers** interleaved between the LLM's self-attention layers: $\mathbf{h}' = \mathbf{h} + \tanh(\alpha) \cdot \text{CrossAttn}(\mathbf{h}, \text{visual\_tokens})$ where $\alpha$ is a learned scalar initialized to 0, ensuring the LLM starts in its original text-only behavior. Text causally attends to the most recent preceding image. This design allows few-shot multimodal prompting — interleave example (image, caption) pairs in the context before the query image, and the model generalizes within context.

## Q7: What is the difference between early fusion and late fusion in multimodal models?

**Early fusion** concatenates or combines raw or low-level features from different modalities before any deep processing: both image pixels and text tokens are processed together from the first layer. This allows the model to learn fine-grained cross-modal interactions but requires synchronized processing of all modalities and cannot reuse pretrained unimodal encoders efficiently. **Late fusion** processes each modality independently through its own encoder to produce separate embeddings, then combines the embeddings at the output stage (e.g., concatenation, element-wise sum, or attention-based pooling). CLIP is a pure late-fusion model — image and text are encoded entirely independently, with interaction only via the contrastive loss during training. **Cross-modal attention** (as in Flamingo, LLaVA) is an intermediate approach: modalities are encoded separately but interact deeply through cross-attention layers, achieving better fusion quality than late fusion while maintaining encoder reusability.
