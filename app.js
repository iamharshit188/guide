...
function applyTheme(light) {
  ...
  if (hljsLink) {
    hljsLink.href = "https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/styles/base16/onedark.min.css";
  }
...
function buildDepGraph() {
  ...
  const W      = panel ? (panel.clientWidth - 64 || 800) : 800;
  const H      = Math.max(panel ? panel.clientHeight - 48 : 480, 480);
  const R      = 22;
...
window.addEventListener("resize", () => {
  ...
  if (_depGraphBuilt && gp && !gp.classList.contains("hidden")) {
    buildDepGraph();
  }
});
